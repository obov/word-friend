...
client = MongoClient('mongodb+srv://test:sparta@cluster0.frndh.mongodb.net/?retryWrites=true&w=majority', tls=True,
...
def home():
    ...
    if logindata == 'login':
        fav = [{'value': 'hi', 'favorite': False, 'complete': True},
               {'value': 'hello', 'favorite': False, 'complete': True},
               {'value': 'how', 'favorite': True, 'complete': False},
               {'value': 'are', 'favorite': True, 'complete': True},
               {'value': 'you', 'favorite': True, 'complete': True}]
        words_for_fav = '-'.join(word['value'] for word in fav)
        recent = [{'value': 'I', 'favorite': False, 'complete': True},
                  {'value': 'am', 'favorite': False, 'complete': True},
                  {'value': 'fine', 'favorite': True, 'complete': False},
                  {'value': 'thank', 'favorite': True, 'complete': True},
                  {'value': 'you', 'favorite': True, 'complete': True}]
        words_for_recent = '-'.join(word['value'] for word in recent)
        recap = [{'value': 'and', 'favorite': True, 'complete': True},
                 {'value': 'you', 'favorite': True, 'complete': True}]
        words_for_recap = '-'.join(word['value'] for word in recap)
        return render_template('index.html', words_for_fav=words_for_fav, words_for_recent=words_for_recent,
                               words_for_recap=words_for_recap)
    else:
...
def bs4(keyword):
    if keyword.isalpha():
        try:
            doc = []
            data = requests.get("https://dic.daum.net/search.do?q=" + keyword + "&dic=eng&search_first=Y",
                                headers=headers)
...
@app.route("/date_word", methods=["get"])
def date_word():
